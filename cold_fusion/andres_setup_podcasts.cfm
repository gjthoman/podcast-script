...
<cfset andresItunesPodcastFile = "#expandPath('/media/audio/podcast/andres_podcast_itunes.xml')#"/>
<cfset andresItunesPodcastUrl = "http://a1611.g.akamai.net/f/1611/23422/9h/dramsey.download.akamai.com/23572/audio/mp3/Andres/"/>
...
<!--- Daveramsey --->
<!--- 
<cfset strFilenameDateSetting = andresItunesPodcastUrl & DateFormat(strDateToCreate,"mmddyyyy")/>
...
<cfset minContentLength = 13000000 />
	<cftry>
		...
		<cfif filecheck.Responseheader.Status_Code IS 200 AND filecheck.Responseheader["Content-Length"] GT minContentLength>
			<cftry>
				<cfinclude template="andres_itunes_podcast_template.cfm"/>
	...
	<cftry> 
		...
		<cfset strFilenameDateSetting = andresItunesPodcastUrl & DateFormat(strDateToCreate,"mmddyyyy")/>
		<cfset minContentLength = 13000000 />
		<cfset enclosureUrl = strFilenameDateSetting & "_the_andres_gutierrez_show_podcast.mp3"/>
		<cfhttp method="head" result="filecheck" url="#enclosureUrl#"  timeout="2" throwonerror="true"/>
		<cfif filecheck.Responseheader.Status_Code IS 200 AND filecheck.Responseheader["Content-Length"] GT minContentLength>
			<cftry>
				<cfinclude template="andres_itunes_podcast_template.cfm"/>
				<cffile action="write" addnewline="no" file="#andresItunesPodcastFile#" output="#xmlcontent#"/>
...
<cfoutput>
	...
	<span>here is the link to generate today's podcast:<br/><a href="http://www.daveramsey.com/scheduledtasks/podcast/andres_setup_podcasts.cfm">http://www.daveramsey.com/scheduledtasks/podcast/andres_setup_podcasts.cfm</a></span>
	</cfif>
